import 'package:flutter/material.dart';

abstract class AppColors {
  static Color get darkgray => const Color(0xFFBDBDBD);

  static Color get lightgray => const Color(0xFFBDBDBD);

  static Color get darkblue => const Color(0xFF263a44);

  static Color get lightblue => const Color(0xFF48a1af);

  static Color get straw => const Color(0xFF90A4AE);

  static Color get red => const Color(0xFF0091EA);

  static Color get green => const Color(0xFF3b7d02);

  static Color get facebook => const Color(0xFF3b5998);
  static Color get google => const Color(0xFF4285f8);
}
